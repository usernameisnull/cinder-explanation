...
class SchedulerAPI(object):
    ...
    def create_consistencygroup(self, ctxt, topic, group,
        ...
        cctxt = self.client.prepare(version='1.8')  # oslo_messaging.rpc.client._CallContext._prepare
        request_spec_p_list = []
        for request_spec in request_spec_list:
            request_spec_p = jsonutils.to_primitive(request_spec)
            request_spec_p_list.append(request_spec_p)
        # 这个调的是那个方法？->cinder.scheduler.manager.create_consistencygroup
        # config.py:
        # cfg.StrOpt('scheduler_manager',
        #            default='cinder.scheduler.manager.SchedulerManager',
        #            help='Full class name for the Manager for scheduler'),
        return cctxt.cast(ctxt, 'create_consistencygroup',  # oslo_messaging.rpc.client._CallContext.cast
                          topic=topic,
                          group=group,
