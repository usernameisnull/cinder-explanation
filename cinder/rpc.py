...
def init(conf):
    ...
    exmods = get_allowed_exmods()  # [cinder.exception.__name__] + []
    TRANSPORT = messaging.get_transport(conf,
                                        allowed_remote_exmods=exmods,   # a list of modules which a client using this transport will deserialize remote exceptions from
...
# oslo_messaging.Target的一个实例， target.topic = 'cinder-scheduler'
def get_client(target, version_cap=None, serializer=None):
    assert TRANSPORT is not None                            # 这里TRANSPORT必须已经有值了，应该是在init()函数里做的初始化，从配置文件获取Transport实例，里面有transport_url、control_exchange、加载驱动插件，
    serializer = RequestContextSerializer(serializer)
    return messaging.RPCClient(TRANSPORT,   # TRANSPORT? 在上面！
                               target,       # 里面有exchange、topic，是不是就相当于AMQP协议的exchange
